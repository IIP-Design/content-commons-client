...
import { maybeFixQuotes, escapeRegExp } from '../utils';
...
export const queryBuilder = store => {
  ...
  if ( store.filter.date.key !== 'recent' ) {
    if ( store.filter.date.key !== 'custom' ) {
      body.filter( 'range', 'published', { gte: store.filter.date.key } );
