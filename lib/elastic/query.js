...
export const queryBuilder = store => {
  ...
  if ( store.filter.language ) {
    options.push( getLanguageQry( store.filter.language ) );
  }
  ...
  if ( store.search.term && store.search.term.trim() ) {
    ...
    if ( hasSelectedTypes ) {
      // use the selected types from search menu
      qryObj.fields = getQryFields( store.search.postTypes );
    } else {
