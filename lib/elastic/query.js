...
// TO DO: refactor as function has become unweildy and hard to reason about
export const queryBuilder = ( store, user ) => { 
  ...
  if ( store.search.language ) {
    ...
    Object.keys( optionsObj ).forEach( optionType => {
      optionsObj[optionType].options.push( getLanguageQry( store.search.language ) );
    } );
  }
  ...
  if ( store.filter.sources.length ) {
    body.filter( 'terms', 'owner.keyword', store.filter.sources );
  }
  ...
  if ( store.search.term && store.search.term.trim() ) {
    ...
    */
    ...
    // Query all types
      ...
      // Pkgs do not have a category field
      if ( !store.filter.categories.length ) {
    ...
    */
    ...
    // Query all types
      ...
      body.orFilter( 'query_string', 'query', optionsObj.nonPkg.optionsStr );
      // Pkgs do not have a category field
      if ( !store.filter.categories.length ) {
        body.orFilter( 'query_string', 'query', optionsObj.pkgs.optionsStr ); /* 4/2/20 - hot fix for filtering out pkgs on lang selection */
