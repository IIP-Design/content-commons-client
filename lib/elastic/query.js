...
const fields = {
  ...
  'package': ['owner', 'title'],
  document: [
    'owner', 'title^6', 'content.html', 'content.rawText', 'countries.name',
  ],
  graphic: [
    'owner', 'title', 'descPublic.content', 'images.title', 'images.filename', 'categories.name', 'tags.name',
  ],
};
...
export const getAvailableLanguages = item => {
  ...
  switch ( item.type ) {
    ...
    case 'graphic': {
      ...
      // Array of language objects from images
...
export const queryBuilder = ( store, user ) => {
  ...
  /* COUNTRIES */
  if ( countries.length ) {
    options.push( getCountriesQry( countries ) );
  }
