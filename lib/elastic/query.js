...
const fields = {
  ...
  'package': ['owner', 'title'],
  document: [
    'owner', 'title^6', 'content.html', 'content.rawText', 'countries.name', 'use', 'bureaus.name', 'bureaus.offices.name',
  ],
  graphic: [
...
export const queryBuilder = ( store, user ) => {
  ...
  if ( bureausOffices.length ) {
    const { list: bureausOfficesList } = store.global.bureausOffices;

    bureausOffices.forEach( bureauOffice => {
      // Get the bureauOffice from the global list to check if selection is a bureau
      const bureauOfficeItem = bureausOfficesList.filter( bo => bo.display_name === bureauOffice )[0];

      if ( bureauOfficeItem.isBureau ) {
        body.orFilter( 'terms', 'bureaus.name.keyword', bureausOffices );
      } else {
        body.orFilter( 'terms', 'bureaus.offices.name.keyword', bureausOffices );
      }
    } );
  }
