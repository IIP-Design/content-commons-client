...
export const languagesRequest = () => axios
  .post( SEARCH, {
    body: bodybuilder()
      .size( 200 )
      .query( 'query_string', 'query', 'type: language' )
      .build(),
  } )
  .then( response => response.data );
...
export const categoryPrimaryRequest = () => axios
  .post( SEARCH, {
    body: bodybuilder()
      .size( 100 )
      .query( 'query_string', 'query', 'type: term AND primary: true' )
      .build(),
  } )
  .then( response => response.data );
...
export const postTypeAggRequest = user => {
  const options = ( user && user.id !== 'public' )
    ? {}
    : { exclude: [
      'document', 'package', 'playbook', 'owner', 'language', 'term',
    ] };
 ...
 * @param {string} operator operator that precedes query string
...
export const typePrioritiesRequest = ( term, categories, tags, currentLang, user ) => {
  ...
  if ( term?.length || categories?.length || tags?.length ) {
    ...
    // if we do not have a user, do not query for documents
    if ( !user?.email ) {
      query += ' AND NOT ( type.keyword: document )';
    }
  }
  ...
  return axios
    .post( SEARCH, {
      body: bodybuilder()
        ...
        // Filter out packages
        .notQuery( 'match', 'type.keyword', 'courses' )
        .notQuery( 'match', 'type.keyword', 'package' )
        .notQuery( 'match', 'type.keyword', 'playbook' )
