 ...
 * @param {*} currentLang
 */
export const typeRecentsRequest = async ( currentType, currentLang, user ) => {
  let body = {};
 ...
 * @param {*} type string
 */
export const typeRequestDesc = async ( type, user ) => {
  const response = await axios
    .post( SEARCH, {
      body: bodybuilder()
        .size( 4 )
 ...
 * @param {*} currentLang
 */
export const typePrioritiesRequest = async ( term, categories, currentLang, user ) => {
  const fields = [
    'title^2',
    'author',
    'content',
