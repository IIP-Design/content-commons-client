 ...
 * Get all post types that have associated content
 */
export const countriesAggRequest = () => axios
  .post( SEARCH, {
    body: bodybuilder()
      .size( 0 )
      .agg( 'terms', 'countries.name.keyword', {}, 'countries' )
      .build()
  } )
  .then( response => response.data )
 ...
 * @param {*} currentLang
 */
export const typeRecentsRequest = ( currentType, currentLang, user ) => axios
  .post(
    SEARCH, {
      body: bodybuilder()
        .size( 3 )
 ...
 * @param {*} currentLang
 */
export const typePrioritiesRequest = ( term, categories, currentLang, user ) => {
  const fields = [
    'title^2',
    'author',
    'content',
