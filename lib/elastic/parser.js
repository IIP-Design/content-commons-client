...
const populateVideoItem = ( source, language ) => {
  ...
  const languageUnitsWithSomeNonClean = units.filter( unit => {
  ...
  const languageUnitThatMatchesParam = languageUnitsWithSomeNonClean.find( unit => unit.language.locale.toLowerCase() === language.toLowerCase() );

  /**
   * Fallback to first language unit with a non-clean video
   * since a project can't be published with all clean videos.
   */
  const languageUnit = languageUnitThatMatchesParam || languageUnitsWithSomeNonClean[0];
...
const getTypeSpecObj = ( source, language ) => {
  ...
  switch ( source.type ) {
    ...
    case 'package':
      ...
      break;
    case 'playbook':
      obj = populatePlaybookItem( source );
      break;
    case 'graphic':
 ...
 */
export const getElasticHitTotal = ( { hits } ) => {
  // if hits is undefined, just return 0;
  if ( !hits ) return 0;
