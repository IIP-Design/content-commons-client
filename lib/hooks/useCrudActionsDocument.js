...
export const useCrudActionsDocument = ( { pollQuery, variables } ) => {
   ...
   * it is UNAVAILABLE indicating a processing error
  ...
  const createFile = async ( pkg, file, callback ) => {
    ...
    try {
      ...
      if ( file.input ) {
        _file = await uploadFile( pkg.assetPath, file, callback );
      }
  ...
  const saveFiles = async ( pkg, files, progressHandler ) => {
    const { toSave, toRemove } = files;

    if ( toRemove?.length ) {
      await Promise.all( toRemove.map( file => deleteFile( pkg, file ) ) );
    }
    ...
    if ( toSave?.length ) {
      return Promise.all(
        toSave.map( file => {
          if ( file.input ) {
            return createFile( pkg, file, progressHandler );
          }
