...
export default ( state = INITIAL_STATE, action ) => {
  switch ( action.type ) {
    ...
    case FILTER_BUREAU_OFFICE_CHANGE:
      // if there is no payload, clear selected bureaus/offices
      if ( !action.payload ) {
        return { ...state, bureausOffices: [] };
      }

      if ( typeof action.payload === 'string' ) {
        return { ...state, bureausOffices: [action.payload] };
      }
