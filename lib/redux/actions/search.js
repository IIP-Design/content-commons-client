...
import { getElasticHitTotal } from 'lib/elastic/parser';
...
export const createRequest = user => async ( dispatch, getState ) => {
  ...
  return dispatch( {
    ...
    payload: {
      response,
      ...calculatePages( getElasticHitTotal( response ), 1, pageSize ), // currentPage
      pageSize,
      currentTerm: term,
    },
  } );
...
export const targetRequest = ( page, user ) => async ( dispatch, getState ) => {
  ...
  return dispatch( {
    ...
    payload: {
      response,
      current: page,
      ...calculatePages( getElasticHitTotal( response ), page, pageSize ),
    },
  } );
};
...
export const sortRequest = ( sortType, user ) => async ( dispatch, getState ) => {
  ...
  return dispatch( {
    ...
    payload: {
      response,
      ...calculatePages( getElasticHitTotal( response ), 1, pageSize ),
    },
  } );
};
...
export const updateSizeRequest = ( newSize, user ) => async ( dispatch, getState ) => {
  ...
  return dispatch( {
    ...
    payload: {
      response,
      ...calculatePages( getElasticHitTotal( response ), 1, newSize ), // currentPage
    },
  } );
};
