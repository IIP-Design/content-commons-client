export const fetchQueryString = state => {
  ...
  if ( state.countries && state.countries.length ) {
    query.countries = state.countries;
  }

  if ( state.documentUses?.length ) {
    query.documentUses = state.documentUses;
  }

  if ( state.bureausOffices?.length ) {
    query.bureausOffices = state.bureausOffices;
  }
