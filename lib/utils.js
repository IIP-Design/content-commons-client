...
export const maybeGetPathToS3 = url => {
  if ( url.startsWith( 'http:' ) || url.startsWith( 'https:' ) ) return url;
  const { publicRuntimeConfig } = getConfig();
  return `https://${publicRuntimeConfig.REACT_APP_AWS_S3_PUBLISHER_UPLOAD_BUCKET}.s3.amazonaws.com/${url}`;
};

export const getPathToS3Bucket = () => {
  const { publicRuntimeConfig } = getConfig();
  return `https://${publicRuntimeConfig.REACT_APP_AWS_S3_PUBLISHER_UPLOAD_BUCKET}.s3.amazonaws.com`;
};
