...
export const getPathToS3Bucket = () => {
  ...
  return `https://${publicRuntimeConfig.REACT_APP_AWS_S3_AUTHORING_BUCKET}.s3.amazonaws.com`;
};

export const getS3Url = assetPath => `${getPathToS3Bucket()}/${assetPath}`;
...
/**
 * Get the API URL for downloading a video from the production S3 bucket.
 * This prevents the video from automatically playing as a stream instead of a download.
 * @param url
 * @param filename
 * @returns {string}
 */
export const getVideoDownloadLink = ( url, filename ) => {
  const { publicRuntimeConfig } = getConfig();
...
export const getTransformedLangTaxArray = ( taxonomy = [], locale = 'en-us' ) => {
  ...
  return taxonomy.reduce( ( acc, tax ) => {
    const langTax = tax && tax.translations && tax.translations.find( t => t.language.locale === locale ) || [];
