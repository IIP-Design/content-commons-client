...
export const numberWithCommas = total => {
...
export const serializeFile = file => new Promise( ( resolve, reject ) => {
  ...
  return reader; // eslint-disable-line no-promise-executor-return
} );
...
export const getLangTaxonomies = ( array, locale = 'en-us' ) => {
  ...
  return (
    array.map( tax => tax.translations
      .find( translation => translation?.language?.locale && translation.language.locale === locale )
      ?.name ).join( ', ' )
  );
};
 ...
 */
export const parseToSpans = text => {
  try {
    return text.split( '\n' ).map( ( br, i ) => (
      <span key={ `index${i + 1}` } style={ { display: 'block' } }>
        { br }
      </span>
    ) );
  } catch ( err ) {
 ...
 * @param {string} text text to be parsed
 */
export const parseToParagraphs = text => {
  try {
    return text.split( '\n\n' ).map( ( para, i ) => <p key={ `index${i + 1}` }>{ parseToSpans( para ) }</p> );
  } catch ( err ) {
    return text;
  }
};
