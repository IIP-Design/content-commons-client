...
const buildVideoFileTree = files => {
...
/*
 * Video Project Edit - Add Video Files
 * Create a Video Unit with new file
 */
export const buildCreateUnitFilesTree = ( projectTitle, file ) => ( {
  units: {
    create: {
      title: projectTitle,
      language: { connect: { id: file.language } },
      files: {
        create: {
          language: { connect: { id: file.language } },
          filename: file.name,
          filetype: file.input.type,
          filesize: file.input.size,
          use: { connect: { id: file.use } },
          quality: file.quality,
          videoBurnedInStatus: file.videoBurnedInStatus
        }
      }
...
/*
 * Video Project Edit - Add Video Files
 * Update a Video Unit with new file
 */
export const buildUpdateUnitFilesTree = ( unitId, file ) => ( {
  units: {
    update: {
      where: { id: unitId },
      data: {
