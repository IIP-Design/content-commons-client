...
/*----------------------------------------
 Fragments
 -----------------------------------------*/
export const GRAPHIC_PROJECT_FRAGMENT = gql`
...
export const GRAPHIC_PROJECT_QUERY = gql`
  query GraphicProject($id: ID!) {
    graphicProject(id: $id) {
      ...graphicProjectDetails
      supportFiles(orderBy: filename_ASC) {
        ...supportFileDetails
      }
    }
  }
        ...
        AND: [
          { team: $team }
          {
            OR: [
              ...
              { descPublic: { content_contains: $searchTerm } }
              { descInternal: { content_contains: $searchTerm } }
