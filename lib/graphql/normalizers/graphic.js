 ...
 * @param {function} autoSelect function that set default dropdown value
 */
export const normalize = ( files, autoSelect ) => {
  let fileList = files;
  const isFunc = typeof autoSelect === 'function';
  ...
  return fileList.map( file => {
    ...
    if ( isFile ) {
      return {
        ...
        name: file.name,
        size: file.size,
        language: isFunc ? autoSelect( 'language', file.name ) : '',
        style: isFunc ? autoSelect( 'style', file.name ) : '',
        social: isFunc ? autoSelect( 'social', file.name ) : [],
        input: file,
        loaded: 0,
      };
    }
